...
docker exec -d $CONTAINER_ID bash -c "source /opt/ros/humble/setup.bash && MicroXRCEAgent udp4 -p 8888"
sleep 2
...
docker exec -d $CONTAINER_ID bash -c "source ~/workspace/install/setup.bash && ros2 run semantic_nav_pkg autopilot_node"
